/*
  ...
  1. Database Trigger Optimization
    - Simplify the user creation trigger function
    - Remove unnecessary error handling that might cause delays
    - Optimize the trigger to run faster
  ...
  3. RLS Policy Optimization
    - Streamline RLS policies for faster execution
    - Remove redundant policy checks

  This migration fixes slow account creation by optimizing the database trigger
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_id_fast ON public.users(id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_progress_user_id_fast ON public.user_progress(user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_achievements_user_id_fast ON public.user_achievements(user_id);
...
-- Create streamlined RLS policies
